...
\textbf{This result is in contradiction to the findings of \citet{baxter2017halo}, who find no splashback feature in the blue galaxy population. At the current state it is not clear how those different results emerge. It might be that, in contrast to our SZ selected cluster sample, there is simply no splashback feature present in the galaxy cluster sample used by \citet{baxter2017halo}, which was constructed using the REDMAPPER algorithm on SDSS data. Further, the separation of the galaxies by color used by \citet{baxter2017halo} is fundamentaly different from our approach. They use K-corrected magnitudes from the SDSS dataset, whereas we avoid using K-corrections but instead use a redshift dependent color cut.}
