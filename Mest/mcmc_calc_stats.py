...
def MCMC_calc_stats(ndim,rank,type_,add,mc):
    ...
    innerrhodev_array = np.zeros((1,rrange.size))
    innerrhodev2_array = np.zeros((1,rrange.size))
    ...
    maxinnerrhodev2_array=np.zeros((1,1))
    ...
    for i in range(chainstate.shape[0]):
		...
		sigma=tools.surf_den(rrange,chainstate[i,j,:])
            ...
            innerrhodev=tools.logdev_einasto_profile_inner(rrange,pact)
            innerrhodev_array=np.vstack((innerrhodev_array,innerrhodev))
            innerrhodev2=tools.logdev2_einasto_profile_inner(rrange,pact)
            innerrhodev2_array=np.vstack((innerrhodev2_array,innerrhodev2))
            ...
            maxinnerrhodev2=tools.logdev2_einasto_profile_inner(rsp3d,pact)
            maxrhodev_array=np.vstack((maxrhodev_array,maxrhodev))
            maxinnerrhodev_array=np.vstack((maxinnerrhodev_array,maxinnerrhodev))
            maxinnerrhodev2_array=np.vstack((maxinnerrhodev2_array,maxinnerrhodev2))
    out=np.hstack((sigma_array, rho_array, dev_array, rhodev_array, rhodev2_array, innerrhodev_array, innerrhodev2_array, rsp2d_array, rsp3d_array, chisquare_array, maxrhodev_array, maxinnerrhodev_array, maxinnerrhodev2_array))
    out=out[1:,:]
