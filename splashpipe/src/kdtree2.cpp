    ...
    ind(N)
{
  ...
  if (rearrange) {
    ...
    // Be careful while changing this to true, the destructor can destroy an
    // already de-allocated memory
    bool inplace=false;
    if(!inplace){
...
KDTree::~KDTree() {
  delete root;
  delete []rearranged_data; //This assumes that inplace will always be false which it is
}
...
KDTreeNode* KDTree::build_tree_for_range(int l, int u, KDTreeNode* parent) {
  ...
  if ((u-l) <= bucketsize) {
    ...
    if (u-l>100000) {
	  ...
	  float diff=(node->box[c].upper-node->box[c].lower)/Nc;
	for (int k=l; k <= u; k++) {
...
void KDTree::r_nearest_brute_force(std::vector<float>& qv, float rr, KDTreeResultVector& result) {
...
SearchRecord::SearchRecord(std::vector<float>& qv_in, KDTree& tree_in,
