...
void cosmology::ukinit2()
{
    ...
    uk_krs = new double[Nuk];
    uk_c = new double[Nuk];
    ukrsc = new double[Nuk*Nuk];
    FILE *fp;
	...
	{
	    double jk1,jk2;
	    if(fscanf(fp,"%lg %lg %lg",&jk1,&jk2,&(ukrsc[i*Nuk+j])) != 3){
...
void cosmology::ukinit()
{
	...
	for(int j=0;j<Nuk;j++)
	{
		...
		ukrsc[i*Nuk+j]=log10(res1/fac);
	    //std::cout<<std::scientific;
	    //std::cout<<krs<<" "<<con<<" "<<res1/fac<<" "<<std::endl;
		if (ukrsc[i*Nuk+j] < -6.0) {
		    extrap=true;
		    x_extrap=uk_krs[j];
		    y_extrap=ukrsc[i*Nuk+j];
		}
	    }else{
		ukrsc[i*Nuk+j]=y_extrap-1.99*(uk_krs[j]-x_extrap);
	    //std::cout<<std::scientific;
	    //std::cout<<krs<<" "<<con<<" "<<pow(10.,ukrsc[i*Nuk+j])<<" "<<std::endl;
	    }
	}
	//std::cout<<std::endl;
    }
...
double cosmology::ukinterp(double krs, double c)
{
    ...
    double y11=ukrsc[ i*Nuk+j ];
    double y12=ukrsc[ i*Nuk+j+1];
    double y21=ukrsc[(i+1)*Nuk+j ];
    double y22=ukrsc[(i+1)*Nuk+j+1];
