...
if __name__ == "__main__":
    ...
    for ii, fname in enumerate(glob.glob("%s/matched_???" % input_dir)):
        print(fname)
        dfnew = pandas.read_csv(fname, delim_whitespace=1, header=None,names=["zred","red","green","red_PS","green_PS","iband_PS"])
    ...
    np.savetxt("%s/matched_spec.dat" % output_dir, np.hstack((zred.reshape(zred.size,1),red.reshape(red.size,1),green.reshape(green.size,1),red_PS.reshape(red_PS.size,1),green_PS.reshape(green_PS.size,1),iband_PS.reshape(iband_PS.size,1))) )
